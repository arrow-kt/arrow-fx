 ...
 */
class CoroutineInfo internal constructor(ctx: CoroutineContext, delegate: DebugCoroutineInfoImpl) {
  /**
   * [Coroutine context][coroutineContext] of the coroutine
   */
  val context: CoroutineContext = ctx

  /**
   * Last observed state of the coroutine
   */
  val state: State =
    State.valueOf(delegate.state)

  private val creationStackBottom: CoroutineStackFrame? =
    delegate.creationStackBottom

  val dispatcher: String? =
    context[ContinuationInterceptor]?.toString()

  val lastObservedThreadState: String? =
    delegate.lastObservedThread?.state?.toString()
