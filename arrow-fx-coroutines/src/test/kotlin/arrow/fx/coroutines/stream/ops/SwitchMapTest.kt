...
class SwitchMapTest : StreamSpec(iterations = 250, spec = {
      ...
      Stream.effect {
        ...
        s.switchMap {
          Stream.effect {
            ...
            if (!released) Stream.raiseError(Throwable())
            else
              Stream.effect {
                ref.set(false)
  ...
  "when inner stream fails, overall stream fails"  {
    checkAll(Arb.stream(Arb.int()), Arb.throwable()) { s0, e ->
      val s = Stream(0).append { s0 }
        ...
        .flatMap { Stream.raiseError<Int>(e) }
        .compile()
  ...
  "when primary stream fails, overall stream fails and inner stream is terminated" {
    checkAll(Arb.throwable()) { e ->
      val s = Stream.effect { Semaphore(0) }
        .flatMap { semaphore ->
          Stream(0)
  ...
  "when inner stream fails, inner stream finalizer run before the primary one" {
    checkAll(Arb.stream(Arb.int()), Arb.throwable()) { s0, e ->
      ...
      val s2 = Stream.effect {
        Promise<Boolean>() to Atomic(false)
      }.flatMap { (verdict, innerReleased) ->
  ...
  "when primary stream fails, inner stream finalizer run before the primary one"  {
    checkAll(Arb.throwable()) { e ->
      val s = Stream
        .effect {
          Atomic(false) to Atomic(false)
        }.flatMap { (verdict, innerReleased) ->

          Stream(1).append { Stream.sleep_(25.milliseconds) }.append { Stream.raiseError(e) }
            .onFinalize {
