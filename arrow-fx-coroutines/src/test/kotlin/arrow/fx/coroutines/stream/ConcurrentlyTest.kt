...
class ConcurrentlyTest : StreamSpec(spec = {
  ...
  "concurrently" - {
    ...
    "run finalizers of background stream and properly handle exception" {
      ...
      val s = Stream(1, 2).append { Stream.never() }
      ...
      val runner = Stream.bracket(
        ...
        }).effectMap { // flag the concurrently had chance to start, as if the `s` will be empty `runner` may not be evaluated at all.
        halt.complete(Unit) // immediately interrupt the outer stream
      }
      ...
      val r = Either.catch {
        Stream.bracket({ Unit }, { finRef.update { it + "Outer" } })
          ...
          .interruptWhen { Either.catch { halt.get() }.also { println("Halted") } }
          .compile()
          .toList()
      }
