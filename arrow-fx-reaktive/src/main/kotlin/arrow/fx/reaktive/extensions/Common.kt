...
internal fun <F, A, B> CoroutineContext.racePair(fa: Maybe<A>, fb: Maybe<B>, fiberFactory: FiberFactory<F>): Maybe<RacePair<F, A, B>> =
  asScheduler().let { scheduler ->
    maybe<RacePair<F, A, B>> { emitter ->
      val connectableA: ConnectableObservable<A> = fa.asObservable().replay(bufferSize = 1)
      val connectableB: ConnectableObservable<B> = fb.asObservable().replay(bufferSize = 1)
...
internal fun <F, A, B, C> CoroutineContext.raceTriple(fa: Maybe<A>, fb: Maybe<B>, fc: Maybe<C>, fiberFactory: FiberFactory<F>): Maybe<RaceTriple<F, A, B, C>> =
  asScheduler().let { scheduler ->
    maybe<RaceTriple<F, A, B, C>> { emitter ->
      val observableA: ConnectableObservable<A> = fa.asObservable().replay(bufferSize = 1)
      val observableB: ConnectableObservable<B> = fb.asObservable().replay(bufferSize = 1)
      val observableC: ConnectableObservable<C> = fc.asObservable().replay(bufferSize = 1)
